...
class MODEL(nn.Module):
    def __init__(self,
        ...
        # Define a linear projection layer for DINO embeddings
        self.dino_projection = nn.Linear(256, hidden_dim)
    ...
    def Align_encoder(self, image_embeddings, DINO_embeddings):
        # Step 1: Project DINO_embeddings to match the dimension of image_embeddings
        dino_proj = self.dino_projection(DINO_embeddings.view(-1, DINO_embeddings.shape[-1]))
        dino_proj = dino_proj.view(DINO_embeddings.shape[1], -1, self.hidden_dim)

        # Step 2: Concatenate projected DINO features with image embeddings
        image_embeddings = image_embeddings.unsqueeze(1)  # [16, 512] -> [16, 1, 512]
        fusion_input = torch.cat((image_embeddings, dino_proj), dim=1)  # [16, 1+900, 512]

        # Step 3: Pass through transformer encoder
        fusion_output = self.transformer_encoder(fusion_input)

        # Step 4: Extract fused features aligned with original image_embeddings
        aligned_features = fusion_output[:, 0, :]  # Output shape: [16, 512]
...
def build_model(name: str):
    ...
    # with open('./models_CLIP/model_config.yml', 'r') as f:
